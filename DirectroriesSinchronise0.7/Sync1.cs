...
namespace DirectroriesSinchronise0._6
{
    public class Sync
    {
        ...
        private string[] parasite = { "Thumbs.db", "Desktop.ini" };
        ...
        public void Comparing()
        {
            ...
            for (int i = 0; i < disk.Length; i++)
            {
                ...
                WriteLn(String.Format("FileSystem {0} Finded", (i + 1).ToString()));
            }
            for (int i = 0; i < disk.Length; i++)
            {
                int j = (i == 0) ? 1 : 0;
                ...
                CmpTn[i].Text = disk[i];
                WriteLn(String.Format("Differens {0} Finded", (i + 1).ToString()));
            }
            compared = true;
        }
            ...
            for (int i = 0; i < tn.Nodes.Count; i++)
                if (tn.Nodes[i].Checked)
                {
                    ...
                    if (IsFile(tn.Nodes[i]))
                    {
                        ...
                        bool paras = false;
                        for (int j = 0; j < parasite.Length && !paras; j++)
                            if (FtnP == parasite[j])
                            {
                                WriteLn(String.Format("{0} will not copy", parasite));
                                paras = true;
                            }
                        if (paras) continue;
                        if (sufftnp == "")
